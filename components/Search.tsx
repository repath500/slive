...
export const Search: FC<SearchProps> = ({ onSearch, onAnswerUpdate, onDone }) => {
  ...
  const handleStream = async (sources: Source[]) => {
    try {
      const prompt = endent`Provide a 3-7 sentence answer to the query based on the following sources. Be original, concise, accurate, and helpful. Cite sources as [1] or [2] or [3] after each sentence (not just the very end) to back up your answer (Ex: Correct: [1], Correct: [2][3], Incorrect: [1, 2]).
      ...
      {loading ? (
        ...
        <div className="mx-auto flex h-full w-full max-w-[750px] flex-col items-center space-y-6 px-3 pt-32 sm:pt-64">
          <div className="flex items-center">
            <IconBolt size={36} />
            <div className="ml-1 text-center text-4xl">LotusAi</div>
          </div>
